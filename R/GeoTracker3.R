...
#' 
#' @export
geo.trackerNREL <- function(lat, long, long.st, Beta, gama, passo, Hora_do_passo, tempo.inicial, tempo.final, Rastreamento) {
  ...
  if (Rastreamento=="1eixo"){
    ...
    cos.theta.out<-cos(theta.z*pi/180)*cos(Beta.out*pi/180)+sin(theta.z*pi/180)*sin(Beta.out*pi/180)*cos(gama.s*pi/180-gama.out*pi/180)
    cos.theta.out[abs(theta.z)>90]<-0
  } else if (Rastreamento=="2eixos"){
    Beta.out=theta.z+0.000001
    gama.out=gama.s+0.000001
  ...
  lista<-list("theta"=theta.out,"gama.s"=gama.s,"theta.z"=theta.z,"omega"=omega,
              "tempos.solar"=tempos.solar,"h.t"=h.t,"phi"=phi,"HaSol"=HaSol,
              "tempos"=tempos, "omega.1"=omega.1, "omega.2"=omega.2,"Delta.T"=Delta.T,
              "omega.sunset"=omega.sunset, "omega.sunrise"=omega.sunrise , "omega.t"=omega.t,
              "omega.t_delta.t"=omega.t_delta.t, "delta"=delta, "Beta"=Beta.out)
  return (lista)
}
