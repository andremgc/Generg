...
#' @export
Ambientais<- function(modo_radiacao, G.cnb, G.cd, theta.z, G.o, DadosAmbientais, HaSol, AlbedoFixo, TipoAlbedo, VVFixo, TipoVV, TAFixo, TipoTA, Ibn, Id, T.Sintetico, tempos, DadosMensais){ #podem ser adicionadas mais entradas de acordo com outras entradas de dado
 ...
 #   tempos<-as.POSIXlt(tempos)
  ...
  if(TipoVV=="horario"){
    VV<-DadosAmbientais$VV
    VV<-VV[1:length(G.cnb)]
  } else if (TipoVV == "fixo"){
    VV<-VVFixo
  } else {
    VV<-DadosMensais$VV[tempos$mon+1]
  }
